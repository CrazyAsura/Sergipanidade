  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <h1 className="text-4xl font-black text-gray-900">Meus <span className="text-primary">Favoritos</span></h1>
        <Badge variant="outline" className="font-black border-primary/20 text-primary">{favLocs.length} salvos</Badge>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {favLocs.map((loc) => (
                <motion.div key={loc.id} whileHover={{ y: -4 }}>
                  <Card className="overflow-hidden cursor-pointer group border-gray-100 hover:border-primary/20 hover:shadow-xl transition-all" onClick={() => router.push(`/locations/${loc.id}`)}>
                    <div className="relative h-44 overflow-hidden"><img src={loc.image} alt={loc.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      <button onClick={(e) => { e.stopPropagation(); dispatch(toggleFavorite(loc.id)); }} className="absolute top-3 right-3 p-2 rounded-full bg-white/80 backdrop-blur-sm hover:bg-white"><Heart className="h-4 w-4 fill-primary text-primary" /></button>
                    </div>
                    <CardContent className="pt-3"><Badge variant="outline" className="text-primary border-primary/20 text-[10px] font-bold mb-1">{loc.category}</Badge><h3 className="font-bold text-gray-900">{loc.name}</h3><p className="text-sm text-gray-400">{loc.city}</p></CardContent>
                  </Card>
                </motion.div>
              ))}
            </div>
    ...
    <Box sx={{ py: 2, pb: 10 }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <Box>
           <Typography variant="h4" sx={{ color: '#1A202C', fontWeight: 950, letterSpacing: -1.5 }}>
             {tabValue === 0 ? 'Salvos' : 'Ranking'}
           </Typography>
           <Typography variant="body2" sx={{ color: 'text.secondary', fontWeight: 700 }}>
             {tabValue === 0 ? 'Seus destinos favoritos' : 'Os preferidos da galera'}
           </Typography>
        </Box>
        <Chip 
          label={tabValue === 0 ? `${favLocs.length} ITENS` : 'TOP 10'} 
          sx={{ 
            fontWeight: 900, bgcolor: 'rgba(237, 33, 0, 0.1)', 
            color: '#ED2100', height: 28, fontSize: '0.65rem',
            border: 'none', px: 1
          }} 
        />
      </Box>
      ...
      {tabValue === 0 ? (
                ...
                >
                  ...
                  <MuiCardContent sx={{ p: 2, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                      <Typography variant="caption" sx={{ color: '#ED2100', fontWeight: 900, textTransform: 'uppercase', fontSize: '0.6rem', letterSpacing: 1 }}>
                        {loc.category}
                      </Typography>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <IconButton 
                        ...
                        sx={{ bgcolor: 'rgba(237, 33, 0, 0.05)', color: '#ED2100' }}
                      >
                        <Heart size={16} fill="#ED2100" />
                      </IconButton>
                    </Box>
                  </MuiCardContent>
                </MuiCard>
        ...
        <Box sx={{ textAlign: 'center', py: 10 }}>
           <Typography variant="h6" fontWeight={900} color="text.primary">RANKING EM BREVE</Typography>
           <Typography variant="body2" color="text.secondary">Estamos compilando os destinos mais amados de Sergipe.</Typography>
           <MuiButton onClick={() => router.push('/locations')} sx={{ mt: 2 }}>Ver Destinos</MuiButton>
        </Box>
      )}
    </Box>
  );
