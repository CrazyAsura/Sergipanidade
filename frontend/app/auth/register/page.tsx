...
function WebRegister() {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center animate-in fade-in zoom-in duration-500">
      <Card className="w-full max-w-lg border-gray-100 shadow-2xl shadow-primary/10 overflow-hidden rounded-[2.5rem]">
         <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="bg-primary p-10 text-primary-foreground flex flex-col justify-between relative overflow-hidden">
               <div className="relative z-10">
                  <ShieldCheck size={48} className="mb-8 opacity-50" />
                  <h3 className="text-3xl font-black italic uppercase leading-none mb-4">Faça parte <br />do Sergipe <br />Invisível.</h3>
                  <p className="opacity-80 font-medium text-sm leading-relaxed">Crie sua conta e comece a salvar seus lugares favoritos agora mesmo.</p>
               </div>
            ...
            <div className="p-10">
               ...
               <CardContent className="p-0 space-y-6">
                  ...
                  <Button onClick={handleRegister} className="w-full h-12 bg-primary hover:bg-primary/90 text-white font-black rounded-xl transition-all shadow-lg shadow-primary/20">CRIAR CONTA</Button>
...
function MobileRegister() {
  ...
  return (
    <Box sx={{ py: 3 }}>
      ...
      <Typography sx={{ fontSize: '2rem', fontWeight: 950, color: '#1A202C', mb: 1, letterSpacing: -1, lineHeight: 1 }}>Crie sua <br /><span style={{ color: '#ED2100' }}>Conta.</span></Typography>
      ...
      <Stack spacing={2.5}>
        <TextField fullWidth label="Nome Completo" value={name} onChange={(e) => setName(e.target.value)}
          InputProps={{ startAdornment: <InputAdornment position="start"><User size={20} color="#ED2100" /></InputAdornment> }}
        ...
        <TextField fullWidth label="E-mail" value={email} onChange={(e) => setEmail(e.target.value)}
          InputProps={{ startAdornment: <InputAdornment position="start"><Mail size={20} color="#ED2100" /></InputAdornment> }}
          sx={{ '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: 'white' } }} />
        <TextField fullWidth type="password" label="Senha" value={pw} onChange={(e) => setPw(e.target.value)}
          InputProps={{ startAdornment: <InputAdornment position="start"><Lock size={20} color="#ED2100" /></InputAdornment> }}
          sx={{ '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: 'white' } }} />
        
        <FormControlLabel 
           control={<Checkbox defaultChecked sx={{ color: '#ED2100', '&.Mui-checked': { color: '#ED2100' } }} />} 
        ...
        <MuiButton fullWidth variant="contained" onClick={handleRegister} sx={{ py: 2, borderRadius: 4, fontWeight: 950, bgcolor: '#ED2100', shadow: '0 10px 20px rgba(237, 33, 0, 0.2)', '&:hover': { bgcolor: '#C41B00' } }}>CADASTRAR AGORA</MuiButton>
      </Stack>

      <Typography variant="body2" sx={{ textAlign: 'center', mt: 4, fontWeight: 700, color: '#A0AEC0' }}>
         Já é de casa? <Link href="/auth/login" style={{ textDecoration: 'none', color: '#ED2100', fontWeight: 900 }}>Entre aqui</Link>
      </Typography>
    </Box>
  );
}
