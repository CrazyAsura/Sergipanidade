  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-700">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="outline" className="font-black gap-1.5 border-orange-200 text-orange-600 px-4 py-1.5 rounded-full">
          <Sparkles className="h-3 w-3" /> Comunidade Ativa
        </Badge>
      </div>

      {/* Create Post */}
      <Card className="border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-lg shadow-orange-50/50 dark:shadow-none rounded-[2rem] overflow-hidden">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            <ShadAvatar className="h-11 w-11 border-2 border-orange-100 dark:border-slate-800 shrink-0">
            ...
            <textarea
              ...
              className="flex-1 bg-gray-50 dark:bg-slate-800 rounded-2xl px-5 py-4 text-sm border border-gray-100 dark:border-slate-700 text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-200 resize-none min-h-[56px] font-medium"
              rows={posting ? 3 : 1}
            />
          </div>
          <AnimatePresence>
            {posting && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex justify-between items-center mt-4 pt-4 border-t border-gray-50 dark:border-slate-800">
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={handlePost} disabled={!newPost.trim()} className="bg-orange-600 hover:bg-orange-700 text-white font-black rounded-full px-8 shadow-lg shadow-orange-200/50 dark:shadow-none disabled:opacity-50">
                    Publicar
                  </Button>
                </div>
              </motion.div>
                ...
                <Card className="overflow-hidden border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:shadow-xl transition-all duration-300 rounded-[2rem]">
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-4">
                      <ShadAvatar className="h-12 w-12 border-2 border-orange-50 dark:border-slate-800">
                  ...
                  <div className="px-6 py-4 flex items-center justify-between border-t border-gray-50 dark:border-slate-800">
                    <div className="flex gap-1">
                      <Button
                        ...
                        className={`font-black gap-1.5 transition-all ${isLiked ? 'text-orange-500 hover:text-orange-600' : 'text-gray-400 dark:text-slate-500 hover:text-orange-500'}`}
                      >
                        <Heart className={`h-[18px] w-[18px] transition-all ${isLiked ? 'fill-orange-500' : ''}`} /> {p.likes}
                      </Button>
                      <Button variant="ghost" size="sm" className="text-gray-400 dark:text-slate-500 font-black gap-1.5 hover:text-blue-500">
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        className={`transition-colors ${isSaved ? 'text-orange-500' : 'text-gray-300 dark:text-slate-700 hover:text-orange-500'}`}
                      >
                        <Bookmark className={`h-[18px] w-[18px] ${isSaved ? 'fill-orange-500' : ''}`} />
                      </Button>
                      <Button variant="ghost" size="icon" className="text-gray-300 dark:text-slate-700 hover:text-orange-500">
  ...
  return (
    <Box sx={{ py: 2, pb: 12 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        ...
        <Chip icon={<Sparkles size={14} color="#E67E22" />} label="ATIVO" variant="outlined" sx={{ fontWeight: 900, borderColor: 'rgba(230,126,34,0.2)', color: '#E67E22', fontSize: '0.65rem', bgcolor: 'rgba(230,126,34,0.05)', height: 28 }} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 4, borderRadius: 6, border: '1px solid rgba(0,0,0,0.04)', bgcolor: 'white', boxShadow: '0 10px 30px rgba(0,0,0,0.04)' }}>
        <Stack direction="row" spacing={2} alignItems="flex-start">
          <MuiAvatar src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=100&auto=format&fit=crop" sx={{ width: 40, height: 40, border: '2px solid #E67E22', p: 0.2 }} />
          <TextField
            fullWidth
            multiline
            rows={posting ? 3 : 1}
          ...
          {posting && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                ...
                <MuiButton variant="contained" onClick={handlePost} disabled={!newPost.trim()} sx={{ bgcolor: '#E67E22', borderRadius: 4, fontWeight: 900, textTransform: 'none', px: 3, boxShadow: '0 4px 12px rgba(230,126,34,0.3)' }}>
                  Publicar
                </MuiButton>
              </Box>
            </motion.div>
              ...
              <Paper elevation={0} sx={{ borderRadius: 6, border: '1px solid rgba(0,0,0,0.04)', overflow: 'hidden', bgcolor: 'white', boxShadow: '0 10px 30px rgba(0,0,0,0.04)' }}>
                ...
                <Box sx={{ p: 2, px: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(0,0,0,0.02)', mt: 1 }}>
                  <Stack direction="row" spacing={2} alignItems="center">
                    <Box onClick={() => toggleLike(p.id)} sx={{ display: 'flex', alignItems: 'center', gap: 0.75, p: 0.75, px: 1.5, borderRadius: 4, bgcolor: isLiked ? 'rgba(230,126,34,0.15)' : 'rgba(230,126,34,0.05)', cursor: 'pointer', transition: 'all 0.2s', '&:active': { transform: 'scale(0.9)' } }}>
                      <Heart size={18} color="#E67E22" fill={isLiked ? "#E67E22" : "none"} />
      ...
      <Fab onClick={() => setPosting(true)} sx={{ position: 'fixed', bottom: 100, right: 20, zIndex: 1100, background: 'linear-gradient(135deg, #E67E22, #F39C12)', boxShadow: '0 8px 30px rgba(230,126,34,0.4)', width: 60, height: 60 }}>
