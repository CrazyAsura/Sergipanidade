...
const servicos = [
  { icon: Bot, title: 'Guia IA Inteligente', desc: 'Assistente pessoal 24h para tirar dúvidas sobre qualquer canto de Sergipe. Responde em tempo real com inteligência artificial.', link: '/guide', cta: 'CONVERSAR COM IA', color: 'text-orange-500', bg: 'bg-orange-50', ring: 'ring-orange-100' },
  ...
  return (
    <div className="space-y-24 animate-in fade-in duration-700">
      ...
      <section className="text-center max-w-3xl mx-auto space-y-6">
        <span className="inline-block px-4 py-1.5 bg-orange-50 text-orange-600 text-[10px] font-black uppercase tracking-widest rounded-full">Nossos Serviços</span>
        <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-gray-900 dark:text-white uppercase italic leading-none">
          Tudo para sua <span className="text-orange-500">viagem</span> ser inesquecível.
        </h1>
      ...
      <section className="bg-linear-to-br from-slate-900 to-slate-800 rounded-[3rem] p-16 md:p-24 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <span className="inline-block px-4 py-1 bg-orange-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full">Premium</span>
            ...
            <Button onClick={() => router.push('/guide')} className="bg-orange-600 hover:bg-orange-700 text-white font-black h-16 px-10 rounded-2xl shadow-xl text-base">
          ...
          <div className="space-y-6">
            {['Roteiros de fim de semana', 'Viagens em família', 'Aventura no sertão', 'Gastronomia sergipana'].map((item, i) => (
              <div key={i} className="flex items-center gap-4 p-5 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                <div className="h-10 w-10 bg-orange-600 rounded-xl flex items-center justify-center text-white shrink-0">
                  <Star size={18} />
                </div>
...
function MobileServicos() {
  ...
  return (
    <Box sx={{ py: 2, pb: 10 }}>
      <Typography sx={{ color: '#E67E22', fontWeight: 900, fontSize: '0.7rem', letterSpacing: 2, textTransform: 'uppercase', mb: 1 }}>Serviços</Typography>
      <Typography variant="h4" sx={{ fontWeight: 950, letterSpacing: -1.5, lineHeight: 1.1, mb: 1 }}>
        Tudo para sua <span style={{ color: '#E67E22' }}>viagem.</span>
      </Typography>
            ...
            <Paper key={i} elevation={0} onClick={() => router.push(s.link)} sx={{ p: 3, borderRadius: 6, border: '1px solid #f0f0f0', bgcolor: 'white', cursor: 'pointer', '&:active': { transform: 'scale(0.98)' }, transition: 'all 0.2s' }}>
              <Box sx={{ display: 'flex', gap: 2.5 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography sx={{ color: '#E67E22', fontWeight: 900, fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 0.5 }}>
      ...
      <Paper elevation={0} sx={{ p: 4, borderRadius: 6, bgcolor: '#1A202C', color: 'white', textAlign: 'center' }}>
        ...
        <MuiButton onClick={() => router.push('/guide')} fullWidth sx={{ bgcolor: '#E67E22', color: 'white', fontWeight: 950, borderRadius: 4, py: 1.5, '&:hover': { bgcolor: '#D35400' } }}>
          FALAR COM IA
        </MuiButton>
      </Paper>
    </Box>
